...
in
{
  wayland.windowManager.hyprland = {
    ...
    extraConfig = ''
      ...
      gesture = 4, vertical, dispatcher, hyprexpo:expo, toggle
    '';
    ...
    settings = {
      ...
      windowrule = [
        ...
        "tile off,match:class ^(swayosd-server)$"
        "float on, pin on, decorate off, size 200 70, move ((monitor_w/2)-100) (monitor_h*0.85), match:class ^(swayosd-server)$"
        "float on,match:title ^(swayosd-server)$"
        # "pin,^(rofi)$"
        # "pin,^(waypaper)$"
        ...
        "float on,match:class ^(Firefox — Sharing Indicator)$"
        # "move 0 0,^(Firefox — Sharing Indicator)$"
        # "size 700 450,^(Volume Control)$"
        # "move 40 55%,^(Volume Control)$"
        "float on,match:class ^(Picture-in-Picture)$"
        "float on,match:title ^(Picture-in-Picture)$"
        "opacity 1.0 override 1.0 override,match:class ^(Picture-in-Picture)$"
        "pin on,match:class ^(Picture-in-Picture)$"
